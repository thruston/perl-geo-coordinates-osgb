...
# StationName     Lat                  Long         ETRS89_Height  ETRS89_East  ETRS89_North  OSGB36_East  OSGB36_North  ODN_Height
my %test_input = (
...
use Test::More; 
plan tests => 1 + 3 * keys %test_input ;
...
for my $k (sort { $test_input{$a}->[0] <=> $test_input{$b}->[0] }  keys %test_input ) {
    ...
    my ($x,$y)     = @{$test_input{$k}}[5,6];
    
    my $given_grid = sprintf "%.3f %.3f", $x, $y;
