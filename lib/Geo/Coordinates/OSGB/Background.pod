...
Point of Origin to the far south west is to make all grid references 
positive.
...
are omitted from the table of data, and a binary search is used to find
entries instead of a direct look up.  

Earlier versions of the module had the data in a hash, but this was much too slow to load.
Later versions includes a `memo-izing' hash to cache the results of the binary
searches, but this made very little improvement to performance so it was removed.
...
Helmert routines are much faster.  A typical benchmarking run on my development
machine (an old Mac Mini server) using the Apple-supplied Perl 5.16 gave:

    Subroutine          calls per sec  ms per call 
    ----------------------------------------------
    ll_to_grid                   8019        0.125 
    ll_to_grid_helmert          37099        0.027 
    grid_to_ll                   2828        0.354 
    grid_to_ll_helmert          29369        0.034 

None of the routines is really slow, since even C<grid_to_ll> averages less
than 0.4 ms per call, but C<ll_to_grid_helmert> runs about 4 to 5 times faster
than C<ll_to_grid>, and C<grid_to_ll_helmert> more than 10 times faster than
