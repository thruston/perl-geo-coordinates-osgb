...
The tests are probably rather generous to my code because of the caching
effect.  In order to speed up the OSTN02 data lookups, the routines keep a
...
ratio is probably rather exaggerated.  Nevertheless this might still be
