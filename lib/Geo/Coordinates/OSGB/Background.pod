...
projection, you will find the paper called I<Surveying with the National
GPS Network>, especially useful.
...
set to C<+4900-00200/> is that all the vertical grid lines are parallel
...
point, with the easting always given before the northing.  For everyday
use however, the OSGB suggest that grid references need only to be given
within the local 100km square as this makes the numbers smaller.  For
this purpose they divide Britain into a series of 100km squares
identified in pair of letters:  TQ, SU, ND, etc.  The grid of the big
squares actually used is something like this:
...
positive easting difference means the WGS Lat/Lon is to the east of
...
OSTN02 in it.  It will be using either a Helmert transformation, or an
even more approximate Molodenksy transformation to translate from the
WGS84 coordinates it is getting from the satellite to find the OSGB grid
figures.
...
of the OSGM02 geoid module, but since this is not used in this module it
is omitted from the table of data in order to save space.  The table of
data contains 876951 rows with entries for each km intersection between
(0,0) and (700000, 1250000).  However nearly 2/3 of these entries
(actually 567472 of them) refer to places that are more than 10 km away
from the British mainland (either in the sea or in Eire), and these are
set to zero indicating that OSTN02 is not defined at these places.  In
order to save more space, these are omitted from the beginning and end
of each row in the data.

The OSTN02 data is included in the C<OSGB.pm> module after the
C<__DATA__> line, and is read using the magic C<< <DATA> >> file handle.
In my tests this proved to be the fastest way to load all that data, by
a long way. 

There are 1229 rows of data, and each row contains up to 701 pairs of
shift data encoded as pairs of hexadecimal integers representing the
shift in mm.  Leading and trailing zeros are omitted, and the number of
leading zeros omitted is stored in the first three characters of each
row.
...
keys requires 2/3 of the space and runs much faster.
...
latitude represents about 70 km or 44 miles, in northern Scotland it's
...
about 5 metres within the vicinity of the British Isles.  
...
Landranger sheet 47 appears as "A:47" and Explorer sheet 161 as "B:161"
and so on.  As of 2015, the Explorer series of incorporates the Outdoor
Leisure maps, so (for example) the two sheets that make up the map
"Outdoor Leisure 1" appear as "B:OL1E" and "B:0L1W".
