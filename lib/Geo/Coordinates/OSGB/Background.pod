...
*** TODO: integrate me ***
Note that the results from a hand-held consumer GPS unit are unlikely to be
more accurate than plus or minus 5m even with perfect reception.  Most GPS
devices can display the accuracy of the current fix you are getting, but at
best this applies only to the latitude and longitude information they are
getting using the WGS84 ellipsoid.  An additional layer of inaccuracy is 
introduced if you set the unit to display the coordinates in OSGB grid format,
because your unit will be using a quick approximation to transform the 
coordinates instead of the full OSTN02 data set (which is used in these modules).
