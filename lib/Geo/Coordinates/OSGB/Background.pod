...
Helmert routines are much faster.  Benchmarking on my development machine
(an old Mac Mini server) gives:

    Subroutine          calls per sec  ms per call 
    ----------------------------------------------
    ll_to_grid                   3481        0.287     
    ll_to_grid_helmert          33503        0.030     
    grid_to_ll                   1127        0.887 
    grid_to_ll_helmert          27704        0.036   

So none of the routines is really slow, since even C<grid_to_ll> takes less
than 1 ms, but C<ll_to_grid_helmert> runs about 10 faster than C<ll_to_grid>,
and C<grid_to_ll_helmert> more than 20 times faster than C<grid_to_ll>.  `Your
mileage may vary' of course.

The routines have been tested with various versions of Perl, 
including recent versions with the C<uselongdouble> option enabled.
Long doubles do not make any significant difference to the round
trip precision of the routines, Perl's default arithmetic is more than
adequate for the calculations required.  The loss of precision in round 
trip testing appears to be inherent in the formulae published by the OS.
