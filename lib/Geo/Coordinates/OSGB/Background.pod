...
to the iterative approach required but there is no obvious reason.
...
the mathematics.  Either way, that is what is achievable with the OS
...
Helmert routines are a bit faster.  A typical benchmarking run on my development
...
None of the routines is really slow, since even C<grid_to_ll> averages about
0.1 ms per call, but C<ll_to_grid_helmert> runs about 30% faster
than C<ll_to_grid>, and C<grid_to_ll_helmert> nearly 3 times faster than
C<grid_to_ll>.  Using a locally compiled version of Perl 5.22, I see an
improvement of about 10%-15% in general on these numbers.
...
slow everything down by about 10%, and make no significant difference to the
round trip precision of the routines. Perl's default arithmetic is more than
adequate for the calculations required.  The loss of precision in round trip
testing appears to be inherent in the formulae published by the OS.
